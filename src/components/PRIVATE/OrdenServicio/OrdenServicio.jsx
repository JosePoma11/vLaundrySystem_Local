...
const OrdenServicio = ({ mode, action, onAction, iEdit, onReturn, iDelivery }) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    productos: Yup.array()
      ...
      .test('producto', 'Debe haber al menos un producto - Delivery no cuenta', function (value) {
        return value.some((item) => item.producto !== 'Delivery');
      })
      .of(
        Yup.object().shape({
  ...
  useEffect(() => {
    ...
    if (formik.values.factura === true) {
      montoImpuesto = +(subTotal * formik.values.cargosExtras.igv.valor).toFixed(2);
    }

...
import { LS_updateRegistered } from '../../redux/states/service_order';
import { LS_updateCodigo } from '../../redux/states/codigo';
...
import { LS_updateNegocio, messageGeneral } from '../../redux/states/negocio';
...
const PrivateMasterLayout = (props) => {
    ...
    const fetchData = async () => {
      ...
      while (intentosActuales.current <= 3 && !success) {
        try {
          ...
          if (Object.keys(infoPrendas).length === 0) {
  ...
  useEffect(() => {
    ...
    socket.on('server:cDelivery', (data) => {

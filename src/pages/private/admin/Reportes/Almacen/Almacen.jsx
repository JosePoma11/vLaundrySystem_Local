...
const Almacen = () => {
  ...
  const handleDonarAlmacenados = async (Ids) => {
    try {
      ...
      console.error(`Error de almacenamiento - ${error.message}`);
    }
  };
  ...
  const handleDonar = async () => {
    try {
      ...
      await handleDonarAlmacenados(updatedIds);

      const updatedInfoAlmacenados = infoAlmacenados.filter((p) => !updatedIds.includes(p._id));

      setInfoAlmacenados(updatedInfoAlmacenados);
      setInDonation(results);
      setRowSelection([]);
      setOrderSelection([]);
      setOnModal('Donados');
      open();

...
import { GetCuadre, SaveCuadre, GetLastCuadre } from '../../../../redux/actions/aCuadre';
...
import { LS_CancelarDeliveryDevolucion, LS_updateDelivery } from '../../../../redux/states/delivery';
import { LS_updateCuadre } from '../../../../redux/states/cuadre';
// import { LS_updateRegisteredDay } from '../../../../redux/states/service_order';
...
const CuadreCaja = () => {
  ...
  const handleSaved = async (clonedElement) => {
  ...
  const sumaMontos = (clientes) => {
    return clientes.reduce((sum, cliente) => sum + (parseFloat(cliente.total) || 0), 0).toFixed(2);
  };
  ...
  useEffect(() => {
    ...
    dispatch(GetCuadre(datePrincipal.fecha));
    dispatch(GetDeliverysDate(datePrincipal.fecha));
    dispatch(GetGastoDate(datePrincipal.fecha));
    dispatch(GetOrdenServices_Date(datePrincipal.fecha));
  }, [datePrincipal]);
  ...
  useEffect(() => {
    ...
    socket.on('server:cCuadre', (data) => {
      if (datePrincipal.fecha === DateCurrent().format4) {
        dispatch(LS_updateCuadre(data));
        const cuadreLS = JSON.parse(localStorage.getItem('respuesta'));
        if (cuadreLS?.dateCuadre.fecha === data.dateCuadre.fecha) {
              ...
              <HeaderCC>
                ...
                <div className="date-filter">
                  <DatePickerInput
                    ...
                    maxDate={new Date()}
                    minDate={moment('2024-01-11').toDate()}
                    onChange={(date) => {
              ...
              <BodyCC>
                <div className="info-top">
                  ...
                  <div className="finish-balance">
                    ...
                    <div
                      style={{ pointerEvents: handleCompareDates(infoLastCuadre.dateCuadre.fecha) ? 'auto' : 'none' }}
                    >
                      <Nota

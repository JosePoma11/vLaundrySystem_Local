...
import { DateCurrent, formatValue, handleGetInfoPago } from '../../../../../../utils/functions';
  ...
  const handleAnular = (infoAnulacion) => {
    dispatch(
      AnularOrderService({
        id: IdCliente,
        infoCliente,
        infoAnulacion: { infoAnulacion: { ...infoAnulacion, _id: IdCliente }, rol: InfoUsuario.rol },
      })
    ).then((res) => {
      if (res.payload) {
        onClose();
  ...
  const handleEditEntrega = (iDelivery) => {
    ...
    dispatch(
      UpdateOrdenServices_PagoEntrega({
        ...
        preLocation: infoCliente.location,
      })
    ).then((res) => {
      if (res.payload) {
        onClose();

...
const delivery = createSlice({
  ...
  reducers: {
    ...
    LS_CancelarDeliveryDevolucion: (state, action) => {
      state.infoDeliveryDate = state.infoDeliveryDate.filter((item) => item._id !== action.payload);
    },
  },
...
export const { LS_updateDelivery, LS_CancelarDeliveryDevolucion } = delivery.actions;
